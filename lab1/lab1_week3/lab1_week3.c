...
volatile int compose_seq_index = 0;
    ...
    if (key == 0){
        ...
        if (compose_mode_record){
            if (compose_array_length > 0){
                ...
                compose_seq_index = 0;
                playback_target = compose_order[0];
                playback_index = 0;
                playback_mode = true;
            }
        }
            ...
            compose_mode_playback = false;
        }
    }
    else if (key >= 1 && key <= 9){
        if (compose_mode_record){
            if (compose_array_length < MAX_COMPOSE_LEN){
        ...
        else if (!record_mode && recording_length[key-1] > 0){
            playback_target = key;
            playback_index = 0;
            playback_mode = true;
        }
            ...
            if (playback_index >= recording_length[playback_target-1]){
                if (compose_mode_playback){
                    ...
                    if (compose_seq_index < compose_array_length){
                        playback_target = compose_order[compose_seq_index];
                        playback_index = 0;
                    }
                    else{
                        compose_mode_playback = false;
